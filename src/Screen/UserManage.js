...
const UserInfoCard = ({ user, showStar }) => {
  ...
  return (
    <div className="user-card">
      ...
      {showStar && <img className="star-icon" src={star} alt="star"/>} {/* Thêm ngôi sao nếu showStar là true */}
      </div>
...
function UserManage() {
  ...
  useEffect(() => {
    const getNewUsers = async () => {
      const response = await fetch("https://api-h89c.onrender.com/users/get-NewUsers");
  ...
  useEffect(() => {
    const getOldUsers = async () => {
      const response2 = await fetch("https://api-h89c.onrender.com/users/get-OdlUsers");
